...
app = Object();
app.server = 'http://demo4.lexita.lt/api';
app.hash = '';
	...
	if(time && localStorage["ekranas.news."+app.home.offset] && (time/1000/60+24) > (+new Date()/1000/60)) {
		...
		app.delegateClick(function(obj) {
			...
			if(type == "article" || type == "gallery") {
				...
				go("'media':'video','url':'rtsp://v8.cache5.c.youtube.com/CjYLENy73wIaLQnFWEAxb4ByZRMYJCAkFEIJbXYtZ29vZ2xlSARSBXdhdGNoYKyL0u2lvLrATQw=/0/0/0/video.3gp'");
			}
		}, '#news-list', '.news');
...
function article(params) {
	...
	if(time && localStorage["ekranas.article."+params[2]] && time/1000/60+24 > +new Date()/1000/60) {
		$('body').append(coco.article({dpi: app.dpi,item: eval('d='+localStorage["ekranas.article."+params[2]])}));
		setTimeout(go("'html':'loaded'"), 500);
	} else {
		$.getJSON(app.server+'/getarticle?id='+params[2]+'&callback=?', function(data) {
			if(!app.apierror) {
				...
				$('body').append(coco.article({dpi: app.dpi,item: data}));
				setTimeout(function() { go("'html':'loaded'"); }, 500);
			}
		});
...
function sezonas() {
	...
	function team() {
		...
		if(time && localStorage["ekranas.team"] && time/1000/60+120 > +new Date()/1000/60) {
			$('body').append(coco.team({dpi: app.dpi,'team': eval(localStorage["ekranas.team"])}));
			app.delegateClick(function(obj) {
			...
			$.getJSON(app.server+'/getteam?callback=?', function(data) {
				if(!app.apierror) {
					...
					$('body').append(coco.team({dpi: app.dpi,'team': data}));
					app.delegateClick(function(obj) {
						go("'go':'player','params':'/id/"+obj.attr('data-id')+"'");
					}, '.white', '.players-list-item');
					setTimeout(function() { go("'html':'loaded'"); }, 500);
		...
		function player(params) {
			...
			if(time && localStorage["ekranas.player."+id] && time/1000/60+120 > +new Date()/1000/60) {
				$('body').append(coco.player({dpi: app.dpi, 'player': eval('d='+localStorage["ekranas.player."+id])}));
				setTimeout(go("'html':'loaded'"), 500);
			} else {
				$.getJSON(app.server+'/getplayer?id='+id+'&callback=?', function(data) {
					if(!app.apierror) {
						...
						$('body').append(coco.player({dpi: app.dpi, 'player': data}));
						setTimeout(function() { go("'html':'loaded'"); }, 500);
					}
				});
	...
	function tables(params) {
		...
		if(time && localStorage["ekranas.tables."+params[1]] && time/1000/60+24 > +new Date()/1000/60) {
			$('body').append(coco.tables({dpi: app.dpi, 'table': eval(localStorage["ekranas.tables."+params[1]])}));
			setTimeout(go("'html':'loaded'"), 500);
		} else {
			$.getJSON(app.server+'/gettables?cat='+params[1]+'&callback=?', function(data) {
				if(!app.apierror) {
					...
					$('body').append(coco.tables({dpi: app.dpi, 'table': data}));
					setTimeout(function() { go("'html':'loaded'"); }, 500);
				}
			});
	...
	function calendar() {
		...
		if(time && localStorage["ekranas.calendar"] && time/1000/60+24 > +new Date()/1000/60) {
			$('body').append(coco.calendar({dpi: app.dpi, 'calendar': eval(localStorage["ekranas.calendar"])}));
			setTimeout(go("'html':'loaded'"), 500);
		} else {
...
function media() {
	...
	function fkelive() {
		...
		$.getJSON(app.server+'/getfkelive?callback=?', function(data) {
			if(!app.apierror) {
				app.apiok = 1;
				$('body').append(coco.fkelive({data:data}));
				setTimeout(function() { go("'html':'loaded'"); }, 500);
			}
		});
...
function photo(params) {
		...
		setTimeout(function() {
			...
			myScroll = new iScroll('scroller', {
				...
				onScrollEnd: function () {
					window.location.hash = "photo/id/"+this.pageX+"/galleryid/"+params[4];
				}
			});
			myScroll.scrollTo(-params[2]*$('body').width(), 0, 10);
			setTimeout(go("'html':'loaded'"), 500);
		}, 200);
	} else {
	...
	function fkemobile() {
		...
		$.getJSON(app.server+'/gettotalizer?callback=?', function(data) {
			if(!app.apierror) {
				app.apiok = 1;
				$('body').append(coco.fkemobile({data: data}));
