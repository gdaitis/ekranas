...
coco.photo = function(opt_data, opt_sb) {
  var output = opt_sb || new soy.StringBuilder();
  output.append('\t<div class=\'photo\' style="width: 100%; height: 100%; overflow: hidden;"><div id="scroller" style="width: 100%; height: 100%; overflow: hidden;"><table style="width: ', soy.$$escapeHtml(opt_data.sw), 'px; height: 100%; margin: 0; padding: 0; border: none; border-collapse: collapse;" cellpadding=0 cellspacing=0><tr>');
  ...
  for (var rowIndex113 = 0; rowIndex113 < rowListLen113; rowIndex113++) {
    ...
    for (var picIndex114 = 0; picIndex114 < picListLen114; picIndex114++) {
      ...
      output.append('<td style="height: 100%; width: ', soy.$$escapeHtml(opt_data.w), 'px; text-align: center; margin: 0; padding: 0; border: none; border-collapse: collapse;" valign="middle"><span id="loading_', soy.$$escapeHtml(picIndex114 + rowIndex113 * 3), '" style="color: #fff;">Kraunama...</span><img class="galimage" style="display: none;" data-src="', soy.$$escapeHtml(picData114.image), '_', soy.$$escapeHtml(opt_data.w - 10), '_', soy.$$escapeHtml(opt_data.h), '" src="" onload="$(\'#loading_', soy.$$escapeHtml(picIndex114 + rowIndex113 * 3), '\').remove(); $(this).css(\'display\',\'inline\');" /></td>');
    }
  }
...
coco.player = function(opt_data, opt_sb) {
  var output = opt_sb || new soy.StringBuilder();
  output.append('\t<div class=\'player white\'><div style="text-align: center;"><img src="', soy.$$escapeHtml(opt_data.player.img), soy.$$escapeHtml(opt_data.dpi), '" /></div><span class="player-name">', soy.$$escapeHtml(opt_data.player.name), '</span><br /><b>Pozicija:</b><br />', soy.$$escapeHtml(opt_data.player.position), '<br /><b>Gimimo data:</b><br />', soy.$$escapeHtml(opt_data.player.born), '<br /><b>Ūgis:</b> ', soy.$$escapeHtml(opt_data.player.height), '<br /><b>Svoris:</b> ', soy.$$escapeHtml(opt_data.player.weight), '<div class="clear"></div><p>', opt_data.player.body, '</p></div>');
  if (!opt_sb) return output.toString();
};
...
coco.tables = function(opt_data, opt_sb) {
  var output = opt_sb || new soy.StringBuilder();
  output.append('\t<table class="table" cellpadding="0" cellspacing="0"><tbody><tr style="color: #999;"><td></td><td></td><td></td><td>R</td><td>P</td><td>L</td><td>Pr</td><td>Įv</td><td>Tšk</td></tr>');
  ...
  for (var statIndex177 = 0; statIndex177 < statListLen177; statIndex177++) {
    ...
    output.append('<tr class="', (statData177.highlight) ? 'high' : '', '"><td style="font-weight: bold;">', soy.$$escapeHtml(statIndex177 + 1), '.</td><td class="tableimg" valign="middle"><img src="', soy.$$escapeHtml(statData177.img), soy.$$escapeHtml(opt_data.dpi), '" /></td><td style="text-align: left; padding-left: 5px;">', soy.$$escapeHtml(statData177.name), '</td><td>', soy.$$escapeHtml(statData177.j), '</td><td>', soy.$$escapeHtml(statData177.g), '</td><td>', soy.$$escapeHtml(statData177.n), '</td><td>', soy.$$escapeHtml(statData177.p), '</td><td>', soy.$$escapeHtml(statData177.diff), '</td><td style="font-weight: bold;">', soy.$$escapeHtml(statData177.pts), '</td></tr>');
  }
...
coco.calendar = function(opt_data, opt_sb) {
  ...
  var monthList206 = opt_data.calendar;
  var monthListLen206 = monthList206.length;
  for (var monthIndex206 = 0; monthIndex206 < monthListLen206; monthIndex206++) {
    var monthData206 = monthList206[monthIndex206];
    if (monthData206.maches) {
      output.append('<div class="month">', soy.$$escapeHtml(monthData206.month), '</div>');
      var matchList212 = monthData206.maches;
      var matchListLen212 = matchList212.length;
      for (var matchIndex212 = 0; matchIndex212 < matchListLen212; matchIndex212++) {
        var matchData212 = matchList212[matchIndex212];
        output.append('<table class="match"><tr><td class="match-team"><img src="', soy.$$escapeHtml(matchData212.leftimg), soy.$$escapeHtml(opt_data.dpi), '"><br />', soy.$$escapeHtml(matchData212.left), '</td><td><span class="match-type">', soy.$$escapeHtml(matchData212.type), '</span><br /><span class="match-date">', soy.$$escapeHtml(matchData212.date), '</span><br /><span class="match-numbers">', soy.$$escapeHtml(matchData212.numbers), '</span></td><td class="match-team"><img src="', soy.$$escapeHtml(matchData212.rightimg), soy.$$escapeHtml(opt_data.dpi), '"><br />', soy.$$escapeHtml(matchData212.right), '</td></tr></table>');
      }
    }
  }
  output.append('</div>');
...
coco.tickets = function(opt_data, opt_sb) {
  ...
  var ticketList241 = opt_data.info.tickets;
  var ticketListLen241 = ticketList241.length;
  for (var ticketIndex241 = 0; ticketIndex241 < ticketListLen241; ticketIndex241++) {
    var ticketData241 = ticketList241[ticketIndex241];
    output.append('<div class="button buy" onclick="go(\'\\\'media\\\':\\\'sms\\\',\\\'number\\\':\\\'1567\\\',\\\'text\\\':\\\'FKEBILIETAS ', soy.$$escapeHtml(opt_data.info.id), ' ', soy.$$escapeHtml(ticketData241.id), '\\\'\');">Pirkti bilietą - ', soy.$$escapeHtml(ticketData241.name), ' - ', soy.$$escapeHtml(ticketData241.price), ' Lt.<br /><span class="aboutsms">SMS numeriu 1567 su tekstu "FKEBILIETAS ', soy.$$escapeHtml(opt_data.info.id), ' ', soy.$$escapeHtml(ticketData241.id), '"</span></div>');
  }
  output.append('</div>');
  if (!opt_sb) return output.toString();
};
...
coco.fkelive = function(opt_data, opt_sb) {
  ...
  if (opt_data.data.event) {
    output.append('<table style="width: 100%; height: 100%;"><tr><td style="width: 100%; height: 100%;"><div style="text-align: center;"><p style="paddign: 10px; margin: 0;">Artimiausios rungtynės:</p><h1>', soy.$$escapeHtml(opt_data.data.event), '</h1><p style="font-weight: bold;">', soy.$$escapeHtml(opt_data.data.date), ' ', soy.$$escapeHtml(opt_data.data.time), '</p><p>', soy.$$escapeHtml(opt_data.data.place), '</p></div></td></tr></table>');
  } else {
    output.append('<div class="live-score"><span>', soy.$$escapeHtml(opt_data.data.score), '</span></div><div class="live-head"><div class="teams"><span style="float: left;">"', soy.$$escapeHtml(opt_data.data.left), '"</span><span style="float: right;">"', soy.$$escapeHtml(opt_data.data.right), '"</span><div class="clear"></div></div></div><div class="clear"></div><div id="live-loader"></div>');
    var commentList278 = opt_data.data.comments;
    var commentListLen278 = commentList278.length;
    for (var commentIndex278 = 0; commentIndex278 < commentListLen278; commentIndex278++) {
      var commentData278 = commentList278[commentIndex278];
      output.append('<div class="live-item"', (commentData278.highlight) ? ' style="font-weight: bold;"' : '', '>', soy.$$escapeHtml(commentData278.text), '</div>');
    }
  }
  output.append('</div>');
  if (!opt_sb) return output.toString();
...
coco.apierror = function(opt_data, opt_sb) {
  var output = opt_sb || new soy.StringBuilder();
  output.append('\t<table id="err" style="width: 100%; height: 100%;"><tr><td valign="middle" style="width: 100%; height: 100%; text-align: center;">Duomenys šiuo metu neprieinami, bandoma dar kartą.<br /><br /><img src="i/loader.gif" /></td></tr></table>');
  if (!opt_sb) return output.toString();
};
